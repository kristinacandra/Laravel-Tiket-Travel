...
@section('content')
    ...
    <div>
        <a href="{{route('tiketpesawat.create')}}" class="btn btn-primary">+ Tambah Data</a>
        <a href="{{route('tiketpesawat.report')}}" rel="noopener" target="_blank" class="btn btn-secondary"><i class="fas fa-print"></i>
            Print</a>
    </div>
    <table class="table table-striped">
        ...
        @foreach ($data as $item)
            <tr>
                ...
                <td class="col-2">
                    <a href="{{route('tiketpesawat.edit',$item->id)}}" class="btn btn-sm btn-warning">Edit</a>
                    <form onsubmit="return confirm('Yakin Ingin menghapus data ini?')" action=" {{ route('tiketpesawat.destroy', $item->id) }}"
